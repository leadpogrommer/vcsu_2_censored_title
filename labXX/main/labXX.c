...
{
    printf("Hello world!\n");
    // auto err = esp_lcd_new_panel_ssd1306()
    ...
    // esp_lcd_panel_draw_bitmap(panel_handle, 1, 1, 5, 5, "g");
    ...
    lv_obj_t *line = lv_line_create(lv_disp_get_scr_act(disp));
    lv_line_set_points(line, (lv_point_precise_t[2]){{0, 0}, {80, 40}}, 2);
    lvgl_port_unlock();
}
